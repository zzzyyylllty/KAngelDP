...
```
# 如果在地牢中，增加元数据
if check kangeldp inside then {
  kangeldp meta add "waves_completed" 1
}

# 获取元数据用于条件判断
if check kangeldp meta get "boss_killed" then {
  tell "Boss 已击杀！"
}

# 在地牢中生成怪物
kangeldp monster spawn "wave_3"
kangeldp tellall "<red>第三波怪物来了！</red>"
kangeldp titleall "<red>⚠ 注意！</red>" "<yellow>Boss 波</yellow>"
```
